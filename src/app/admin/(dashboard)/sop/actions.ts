...
export async function createSopAction(formData: FormData) {
  const s = await admin();
  const title = String(formData.get("title") ?? "").trim() || "Untitled SOP";
  let slug = slugify(title);
  if (await prisma.sopArticle.findUnique({ where: { slug } })) slug = `${slug}-${Date.now().toString(36).slice(-4)}`;
  const a = await prisma.sopArticle.create({ data: { title, slug, ownerUserId: s.sub, category: String(formData.get("category") ?? "") || null, blocks: [] } });
...
export async function saveSopBlocksAction(formData: FormData) {
  ...
  const blocks = JSON.parse(String(formData.get("blocks") ?? "[]")) as Prisma.InputJsonValue;
...
export async function submitSopAction(formData: FormData) {
  await admin();
  await prisma.sopArticle.update({ where: { id: String(formData.get("id")) }, data: { status: "in_review" } });
  redirect(`/admin/sop/${String(formData.get("id"))}`);
}
